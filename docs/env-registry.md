...
| `AFK_GOBLIN_MASCOT` | boolean |  |  | `1` | Reacting goblin mini-sprite tucked at the right edge of the loop-stage footer row while the agent runs tools (1 row shared with the rail, animated). 1 = on, unset/0 = off (default). Claims no terminal rows. |
