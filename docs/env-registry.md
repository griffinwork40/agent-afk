...
| `AFK_GOBLIN_MASCOT` | boolean |  |  | `1` | Reacting goblin sprite in a reserved footer band, right-aligned and present for the whole session (3 rows; animated while the agent runs tools, resting otherwise). 1 = on, unset/0 = off (default). Claims terminal rows, so it is opt-in. |
