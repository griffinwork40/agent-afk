...
- **Background subagent jobs** — dispatching an `agent` tool call with `mode:'background'` detaches the subagent into the `BackgroundAgentRegistry`. `/bgsub` lists running and completed jobs; `/bgsub:join <id>` surfaces a result to the model. Status bar at the bottom of the REPL surfaces running job counts. Implementation: `src/cli/background-status-bar.ts`, `src/agent/background-registry.ts`.
