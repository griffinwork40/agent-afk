...
The working directory for spawned sessions defaults to the current directory. Override the daemon-wide default with `AFK_DAEMON_CWD`, or pin individual tasks to a specific repo with the per-task `cwd` field (see [Scheduled tasks](/docs/surfaces/daemon#scheduled-tasks) for details).

**Working directory precedence:** `task.cwd` → `AFK_DAEMON_CWD` → `process.cwd()`. Setting `cwd` on a task prevents grep/glob calls from walking the entire home directory during cron sweeps.
...
|---|---|---|
| `AFK_MAX_OUTPUT_TOKENS` | model ceiling (64k–128k) | Output tokens per turn only |
| `AFK_MAX_TOKENS` | `4096` | Deprecated / inert — use `AFK_MAX_OUTPUT_TOKENS` |
