...
| 3 | `<cwd>/.mcp.json` | Project-local config — **not loaded by default** (opt-in); set `AFK_ALLOW_PROJECT_MCP=1` to load |
...
Project-local `.mcp.json` is **not loaded by default** — it is opt-in. A project-local config can spawn arbitrary commands on session start, so entering an untrusted repository must never auto-spawn its servers. When a `.mcp.json` is present but the opt-in is absent, AFK skips it and prints a warning listing the skipped server names, their commands, and the exact opt-in instruction. Set `AFK_ALLOW_PROJECT_MCP=1` (or `true`/`yes`/`on`) to load this layer for a directory you trust. A `.mcp.json` whose real path resolves outside the working directory (via a symlink or `..`) is refused outright.
