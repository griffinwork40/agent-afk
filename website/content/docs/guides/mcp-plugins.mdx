...
Agent AFK supports the Model Context Protocol (MCP) for connecting external tools and data sources. Plugins extend AFK's capabilities with skills, commands, and agents. This guide covers both.
...
When two sources define a server with the same name, the higher-priority source wins and the displaced source is surfaced as a warning. Per-name conflicts do not silently drop servers.
...
Marketplace state lives under `~/.afk/marketplaces/`.
