...
Agent AFK has several built-in verification passes that run before changes reach production. This guide covers the tools available and the underlying pattern they share.
...
Internally, AFK uses a confidence gate (`src/skills/_lib/confidence-gate.ts:9-13`) to decide when subagent output needs re-verification. The gate uses a threshold, gap detection, and boundary classification — when any condition fires, the output goes to verification before being acted on.
