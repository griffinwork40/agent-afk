...
description: "Use --format stream-json to get NDJSON output from afk chat, pipe it into scripts, and parse OutputEvents in Node or shell."
---
...
- **AFK-native format** — the NDJSON shape is not translated to any external streaming protocol (e.g. OpenAI's SSE format).
