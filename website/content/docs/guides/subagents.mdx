...
Subagents are full child `AgentSession` instances created by `SubagentManager.forkSubagent()` (`src/agent/subagent.ts`). Each inherits permissions from the parent and runs the same tool surface. What it does not inherit is context — every subagent starts with zero prior conversation.
...
## AbortGraph — abort propagation

Subagents form a tree of `AbortController`s managed by `AbortGraph` (`src/agent/abort-graph.ts`). The rules:
...
See the [Verification guide](/docs/guides/verification) for the full workflow.
