...
credential path is protected whether the agent reaches for `read_file` or for `cat`. The typed-tool
floor is unconditional on every surface; the bash half is not — see
[Bash restrictions](#bash-restrictions) for the headless exception.
...
- **Restricted roots** — commands referencing sensitive paths. This is the same credential list the
  typed read tools refuse (see [Protected paths](#protected-paths) above) plus a few shell-only
  additions: `~/.password-store`, `~/Library/Application Support` (browser profiles), and the whole
  `~/.config/gh` directory. Each `/etc/…` entry is also matched as its `/private/etc/…` twin, and
  vice versa, since macOS reaches the same file by both names. A restricted path is allowed only
  when a granted root already covers it.
