...
- **Restricted roots** — commands referencing sensitive paths. This is the same credential list the
  typed read tools refuse (see [Protected paths](#protected-paths) below) plus a few shell-only
  additions: `~/.password-store`, `~/Library/Application Support` (browser profiles), and the whole
  `~/.config/gh` directory. A restricted path is allowed only when a granted root already covers it.
...
| `AFK_READ_DENYLIST` | Colon-separated absolute paths added to the credential floor above — refused by the typed read tools on **every** surface, and by bash on **interactive** surfaces only ([headless fails open](#bash-restrictions)). Adds only; built-ins cannot be removed. | — |
