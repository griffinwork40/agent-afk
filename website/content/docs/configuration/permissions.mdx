...
Add your own entries with `AFK_READ_DENYLIST` (colon-separated absolute paths). Operator entries are
matched *before* the carve-out, so `AFK_READ_DENYLIST=~/.afk/config/mcp.json` re-protects the
registry if yours holds inline secrets. Built-in entries always apply and cannot be removed.

This list backs the bash restrictions below, so a credential path is protected whether the agent
reaches for `read_file` or for `cat` — bash additionally floors a few wider, shell-only roots (see
below).
...
- **Restricted roots** — commands referencing sensitive paths. This is the same credential list the
  ...
  file) and `/private/etc/sudoers`. `~/Library/Application Support` is *also* wider here than the
  typed floor — bash restricts the whole directory, while `read_file`/`grep`/`glob` restrict only
  the per-browser secret trees inside it (see [Protected paths](#protected-paths)) — because bash's
  floor is grant-liftable (`/allow-dir <path>` drops it for a session) while the typed floor is
  unconditional and permanent. A restricted path is allowed only when a granted root already covers
  it.
...
| `AFK_READ_DENYLIST` | Colon-separated absolute paths added to the credential floor above — refused by the read tools and by bash, on every surface. Adds only; built-ins cannot be removed. | — |
