...
| `AFK_SUBAGENT_RESULT_CAP_BYTES` | number | Foreground subagent result size cap in bytes. When a subagent's final message exceeds this threshold, the full output is spilled to a sidecar file (`state/sessions/&lt;id&gt;/subagent-handoffs/&lt;subagentId&gt;.txt`) and the parent receives a head+tail slice with a `read_file` pointer. Prevents large subagent outputs from bloating parent context. Set to `0` to disable the cap entirely. Note: sidecar files contain unredacted subagent output (same caveats as `AFK_CAPTURE_SUBAGENT_OUTPUT`). | `32768` |
| `AFK_SYSTEM_PROMPT` | string | Raw operator-overlay prompt. Highest-priority overlay (over afk.config.json and AFK.md). Appended on top of the framework base (system-prompt.md) under an "# Operator configuration" header — it augments, never replaces, the base. | — |
