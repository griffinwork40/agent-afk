...
The **ProviderRouter** (`src/agent/providers/router/provider-router.ts`)
handles mid-session switches transparently. Cost/token accumulators and
session lifecycle hooks are owned by the session layer, so a model switch
does not reset budgets or re-fire hooks.
...
the router carries a text-only shadow history forward — the new model sees prior
turns as plain prose, not structured tool calls. Same-family switches keep full
fidelity.
...
[Model Slots](/docs/configuration/model-slots).
...
this flag. See [API Keys](/docs/configuration/api-keys) for OAuth details.
