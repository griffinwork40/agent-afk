...
- `claude-*` model IDs
- Identity aliases (fixed): `opus`/`opus_1m`, `opus-5.5`/`opus-5.5_1m`, `sonnet`/`sonnet_1m`, `haiku`, `fable`
...
| `opus-5.5` / `opus-5.5_1m` | Opus 5.5 — faster, cheaper Opus; extended thinking, adaptive by default | anthropic-direct |
...
```bash
export AFK_THINKING=adaptive      # adaptive | disabled | max | enabled:<N> | enabled:max
