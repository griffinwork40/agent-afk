...
`loadAnthropicCredential()` in `src/agent/auth/credential-resolver.ts` walks:
...
`OPENAI_API_KEY` is the preferred credential. `CODEX_API_KEY` is a **deprecated**
legacy name from the removed `@openai/codex-sdk` integration — it is still read
as a fallback after `OPENAI_API_KEY` but will be removed in a future major release.
Prefer `OPENAI_API_KEY` for all new setups.
...
[Model Slots](/docs/configuration/model-slots) for details.
