...
— `small`, `medium`, and `large`. The tier positions are stable anchors that
the `agent`, `compose`, and `skill` tools select among (cheapest → most
capable); what you bind to each tier is your choice.
...
`providerForModel()` resolves a slot alias to its full binding **before** provider
pattern-matching. This means a tier rebound to a non-Anthropic ID (e.g.
`small → gpt-4o-mini`) automatically routes to `openai-compatible` without any
per-call changes.

Per-slot credentials are applied at session construction via `applySlotCredentials`
in `src/agent/session/slot-credentials.ts`. This covers the main session and
every child dispatch path (subagents, compose DAG nodes, skills) uniformly.
An Anthropic OAuth token cannot leak into an OpenAI-routed tier.
