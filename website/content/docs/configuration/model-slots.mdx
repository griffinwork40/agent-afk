...
- An **identity alias**: `haiku`, `sonnet`/`sonnet_1m`, `opus`/`opus_1m`, `opus-5.5`/`opus-5.5_1m`, `fable`
...
3. Identity alias (`haiku`/`sonnet`/`opus`/`opus-5.5`/`fable`/`*_1m`) → its own fixed model ID — **never** a tier
