...
- **`web_scrape`** — one tool, three modes: fetch a page as markdown, fetch raw bytes, or run a web search. There is no separate `WebFetch`/`WebSearch` — those names are aliases that normalize to `web_scrape`.
