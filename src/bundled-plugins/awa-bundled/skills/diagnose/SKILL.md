...
**TUI label convention.** Set `id_prefix` on each Agent dispatch to a short, descriptive label so the live progress view shows the agent's role:
- Codebase search agent: `id_prefix: "codebase search"`
- Git history agent: `id_prefix: "git history"`
- Each hypothesis-testing agent: `id_prefix: "hypothesis: <one-phrase cause>"` (e.g. `"hypothesis: null deref in parser"`)
