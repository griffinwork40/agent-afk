...
**Merge + surface:**
- Recommendation = `original` → the proposal survived critique; proceed with it. When a steelman annotation exists, surface `gaps_filled` alongside the recommendation — those are the assumptions the proposal depends on but never stated, and they are load-bearing whether or not its author wrote them down. **`original` always denotes the user's verbatim proposal**; report the strengthened text as an annotation on it, never as a silent replacement for it.
