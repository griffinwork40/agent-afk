...
describe('cd-on-exit', () => {
  ...
  it('recordCdIntent swallows write failures (e.g. read-only state dir)', () => {
    ...
    // EPERM, masking the intended swallow-write-failure assertion (#703).
    const absoluteTarget = join(tmpdir(), 'afk-cd-swallow-test');
    // Should not throw — best-effort contract.
    expect(() => recordCdIntent(absoluteTarget)).not.toThrow();
  });
