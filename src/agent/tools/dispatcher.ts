...
export class SessionToolDispatcher implements ToolDispatcher {
      ...
      try {
        ...
        if (err instanceof HookBlockedError) {
          return this.recordForkReadDenial(call, err.reason, {
            content: `Tool "${call.name}" blocked by PreToolUse hook${err.reason ? `: ${err.reason}` : ''}${err.injectContext ? `\n\n${err.injectContext}` : ''}`,
            isError: true,
  ...
  async executeBatch(
    ...
    const batchDeps: BatchExecDeps = {
      ...
      // Live batch-start event sink (Phase 2, issue #516): call onBatchStart
      // directly so the provider generator can yield `tool.batch.start` events
      // DURING the pending window — before any handler result arrives.
      batchEventSink: onBatchStart,
    };
