...
describe('hasValidToolUsePairing (#2004 — only nextUserBlocks, not currentUserBlocks)', () => {
  ...
  it('returns false when the matching tool_result is only in currentUserBlocks (false-positive prevention)', () => {
    ...
    // is never a parameter and is structurally unreachable. This test verifies
    // that an empty next-turn yields `false`, which is the structural guarantee
    // preventing the false-positive that a dual-search function would allow.
