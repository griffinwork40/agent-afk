...
export interface ToolResultChunk {
  ...
  capturePath?: string;
  /**
   * The last up to N lines of the tool's raw output, stored verbatim so the
   ...
   * `lineCount > 1` (multi-line output). N is configured via
   * `AFK_BASH_PREVIEW_TAIL_LINES` (default 7). Each entry is already sanitized
   * for single-line display. TUI-only — never model-facing.
   */
  tailPreview?: string[];
  /**
   * The first up to M lines of the tool's raw output, stored verbatim so the
   * TUI can render an actual head preview in the outcome row. Set when
   * `AFK_BASH_PREVIEW_HEAD_LINES` is non-zero. When head + tail >= total
   * non-empty lines, all lines appear in headPreview and tailPreview is omitted
   * to avoid duplication. TUI-only — never model-facing.
   */
  headPreview?: string[];
