...
export interface ToolResultChunk {
  ...
  capturePath?: string;
  /**
   * The last up to N lines of the tool's raw output (N from
  ...
  tailPreview?: string[];
  /**
   * Optional head block: the first up to M non-empty lines of the tool's raw
   * output (M from `AFK_BASH_PREVIEW_HEAD_LINES`, default 0 = disabled).
   * When present, the TUI renders this block above the hidden-count separator
   * and before the tail block. TUI-only — never model-facing.
   */
  headPreview?: string[];
