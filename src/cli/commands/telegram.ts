  ...
  telegram
    ...
    .action(async (opts: { timeoutSec?: string }) => {
      ...
      if (!Number.isFinite(timeoutSec) || timeoutSec < 1) {
        console.error(palette.error('--timeout-sec must be a positive integer'));
        process.exit(2);
      }
  ...
  telegram
    ...
    .action(async () => {
      ...
      if (result.kind === 'started') {
        console.log(palette.success(`✓ Bot started (PID ${result.pid})`));
        console.log(palette.meta(`  Logs: ${result.logFile}`));
        console.log(palette.meta(`  Tail with: afk telegram logs --follow`));
        return;
      }
      if (result.kind === 'already-running') {
        console.log(palette.warning(`⚠ ${result.message}`));
        process.exit(1);
      }
      if (result.kind === 'exited-immediately') {
        ...
        if (result.logTail && result.logTail.length > 0) {
          console.error('');
          console.error(palette.heading('Last log entries:'));
          for (const line of result.logTail) console.error(palette.meta(`  ${line}`));
        }
        process.exit(1);
      }
      console.error(palette.error(`✗ ${result.message}`));
  ...
  telegram
    ...
    .action(async () => {
      ...
      if (result.kind === 'not-running') {
        console.log(palette.warning('⚠ Bot is not running'));
        return;
      }
      if (result.kind === 'stopped') {
        console.log(palette.success(`✓ Bot stopped (PID ${result.pid})`));
        return;
      }
      console.log(palette.warning(`⚠ Bot force-killed (PID ${result.pid}); graceful shutdown timed out`));
    });
  ...
  telegram
    ...
    .action(async () => {
      ...
      if (stopResult.kind === 'stopped' || stopResult.kind === 'force-killed') {
        console.log(palette.meta(`Stopped (PID ${stopResult.pid})`));
      }
      const startResult = await start();
      if (startResult.kind === 'started') {
        console.log(palette.success(`✓ Bot restarted (PID ${startResult.pid})`));
        return;
      }
      console.error(palette.error(`✗ Restart failed: ${startResult.message}`));
      process.exit(1);
    });
  ...
  telegram
    ...
    .action((opts: { follow?: boolean; lines?: string }) => {
      ...
      if (!existsSync(logFile)) {
        console.log(palette.warning(`No log file at ${logFile}`));
        console.log(palette.meta('Start the bot first: afk telegram start'));
        return;
      }
      ...
      if (opts.follow) {
        ...
        tail.on('error', (e) => {
          console.error(palette.error(`Failed to spawn tail: ${e.message}`));
        });
        return;
      }
...
function printStatus(s: BotStatus): void {
  console.log(palette.heading('📊 Telegram Bot Status'));
  console.log('');
  if (s.running) {
    ...
    console.log(`  ${palette.error('●')} Stopped`);
  }
  console.log(`  PID:     ${s.pidFile}`);
  console.log(`  Logs:    ${s.logFile}`);
  if (s.logTail && s.logTail.length > 0) {
    console.log('');
    console.log(palette.heading('Recent log entries:'));
    for (const line of s.logTail) console.log(palette.meta(`  ${line}`));
  }
}
