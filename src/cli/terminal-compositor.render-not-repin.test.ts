...
describe('Stage 2 (#540) render-not-repin: stateless window re-render', () => {
  ...
  it('keeps a thin band adjacent to the frame on a short terminal (#2182 cap reverted)', async () => {
    // #2182 once capped the blank rows ABOVE a short band at ceil(rows/3) by
    // top-shifting the band and erasing the rows between it and the frame,
    // which opened a mid-screen gap between committed content and the prompt.
    // The cap was reverted: repositionCommittedBand bottom-aligns the band on
    // the frame (zero gap), and the top-blank case #2182 reported is solved by
    // content-hug placement instead (terminal-compositor.content-hug.test.ts),
    // which positions the frame at floor + band.length so no blank rows exist.
    const SHORT_ROWS = 20;
    const stdout = makeStdout();
    ...
    // desiredTopRow=17 → targetBottom=16 → band bottom-aligned at rows 14..16.
    repositionCommittedBand(host, /* desiredTopRow */ 17, /* preRenderFrameTop */ 0, /* targetBottomRow */ 19);

    const term = new HeadlessTerminal({ cols: COLS, rows: SHORT_ROWS, scrollback: 100, allowProposedApi: true, convertEol: true });
    await termWrite(term, all());
    const lines = allLines(term);
    const dump = lines.map((l, i) => `[${String(i).padStart(2)}] ${JSON.stringify(l.replace(/\s+$/, ''))}`).join('\n');

    // 0-based row 15 is 1-based row 16 = targetBottom: the band's last row sits
    // immediately above the frame top (row 17) — no gap between them.
    expect(lines[15] ?? '', `band not adjacent to the frame:\n${dump}`).toContain('RESULT-C');
    expect(lines[13] ?? '', `band not bottom-aligned:\n${dump}`).toContain('RESULT-A');
